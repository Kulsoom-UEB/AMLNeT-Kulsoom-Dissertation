...
This section is written so that the project can be reproduced from a clean machine without prior knowledge of it.

### Step 1 — Prerequisites

- **Python 3.14.5** (the version the results were produced with)
- Roughly **2 GB free disk space** — 691 MB for the dataset, the rest for Parquet intermediates and model files
- **16 GB RAM recommended.** No GPU is needed; every result in the paper was produced on a standard Windows 11 laptop CPU
...
This takes a few minutes. Versions are pinned so the environment matches the one used to produce the reported results.

### Step 5 — Download the dataset

The dataset is **not** in this repository (691 MB exceeds GitHub's 100 MB file limit).
...
This checks your Python and package versions, creates any missing folders, and verifies `AMLNet.csv` against its published MD5 checksum (`7668fc7d74c787e07546ce85c6f790b9`). **If this step fails, stop and fix it before continuing** — a wrong or partial download will otherwise produce misleading results downstream.
...
The script prints a banner and a timing line for each stage, then a total runtime and the output locations.
...
| Step | Stage | Typical runtime | Writes |
|---|---|---|---|
| 00 | Setup and reproducibility | seconds | Environment table, dataset integrity check |
| 01 | Data understanding and leakage audit | 1–3 min | Profiling tables, `fig01`–`fig03` |
| 02 | Leakage removal and cleaning | 1–2 min | `data/processed/cleaned_amlnet.parquet` |
| 03 | Feature engineering | 1–2 min | `data/processed/engineered_amlnet.parquet` |
| 04 | Train / validation / test split | under 1 min | Six split files, split integrity table |
| 05 | Model development and tuning | **10–15 min** | Six tuned pipelines, hyperparameter results |
| 06 | Threshold optimisation and selection | 2–4 min | `models/final_model_config.json`, threshold tables, `fig04`–`fig06` |
| 07 | Final test evaluation | 1–2 min | Test metrics, confusion matrix, `fig07`–`fig08` |
| 08 | Explainability | 3–5 min | Importance tables, `fig09`–`fig12` |

**Total: roughly 25–35 minutes.** Step 05 dominates, because it tunes six model/strategy combinations with 3-fold cross-validation.

> **Why the search budget is small.** Step 05 samples six hyperparameter configurations per model and strategy, and tunes on a 250,000-row stratified subsample rather than the full training set. This is a deliberate hardware accommodation, not an oversight. A larger initial search fitted on all 763,120 training rows exhausted the 16 GB of memory available and terminated the development environment mid-run — the one-hot expanded matrix has to be held across three cross-validation folds and several candidate fits at once. Reducing the budget and subsampling for the search, then refitting the winning configuration on the complete training set, made the six-way comparison finish reliably while still producing cross-validated PR-AUC between 0.925 and 0.959 across all candidates. If you are running this on better-provisioned hardware, `N_ITER` in `src/step05_model_development.py` can safely be raised.

The pipeline is deterministic: seed 42 is fixed across `random`, `numpy`, `PYTHONHASHSEED`, scikit-learn and XGBoost, so a re-run reproduces the same numbers.
...
Opens in your browser at `http://localhost:8501`. See the dashboard section below for what each tab shows.

### Step 9 — Check the results against the paper
...
`outputs/` is committed to this repository, so the reported results can be inspected **without running anything** — running the pipeline regenerates them.
...
| Step 05 seems to hang | Normal — tuning takes 10–15 minutes and prints only at stage boundaries |
...
├── src/                         all source code (see table below)
├── models/                      generated by the pipeline (not committed)
├── outputs/
...
| `amlnet_resampling.py` | shared | The custom SMOTENC pipeline. Orders steps as scale → integer-code categoricals → resample → one-hot, because standard ordering would hand SMOTENC dummy columns and break its majority-vote step |
| `step00_setup.py` | setup | Verifies the environment, pins and reports package versions, creates the folder structure, and checks `AMLNet.csv` against its published MD5 before anything else runs |
...
| `app.py` | Deployment | The Streamlit reviewer dashboard — see the section below |
| `run_all.py` | orchestration | Runs `step00` to `step08` in order as a single command |
| `build_notebooks.py` | optional | Utility that regenerates Colab notebooks from the step modules. Not part of the analysis pipeline; the reported results come from the `src/step*.py` scripts run in PyCharm |

### `outputs/figures/` — 13 figures

`fig01_target_class_distribution`, `fig02_amount_distribution_by_class`, `fig03_laundering_rate_by_category`, `fig03_model_comparison`, `fig04_validation_pr_curve`, `fig05_validation_roc_curve`, `fig06_threshold_optimisation`, `fig07_test_confusion_matrix`, `fig08_test_pr_curve`, `fig09_shap_global_importance`, `fig10_builtin_importance`, `fig11_permutation_importance`, `fig12_local_shap_example`
...
## The reviewer dashboard (`src/app.py`)
...
The last tab is deliberate. The dashboard is a technical prototype for academic demonstration, not a production AML system, and it states that where a user will actually see it.
